...
class DDPG():
    ...
    def build_actor_network(self, in_shape, out_shape, num_layers, layer_size, ):
        ...
        model.add(tf.keras.layers.Dense(
            layer_size, activation="relu", input_shape=(in_shape,)))
    ...
    def build_critic_network(self, state_shape, action_shape, num_layers, layer_size):
        ...
        state_output = tf.keras.layers.Dense(
            layer_size, activation="relu")(state_input)
        action_output = tf.keras.layers.Dense(
            layer_size, activation="relu")(action_input)

        for _ in range(num_layers-1):
            state_output = tf.keras.layers.Dense(
                layer_size, activation="relu")(state_output)
            action_output = tf.keras.layers.Dense(
                layer_size, activation="relu")(action_output)

        merged = tf.keras.layers.Concatenate()([state_output, action_output])
        merged_out = tf.keras.layers.Dense(
            layer_size, activation="relu")(merged)
    ...
    def set_noise(self, state):
        ...
        wandb.log({"Noise": state})

        # turn the noise on and off
        if state == 1:
